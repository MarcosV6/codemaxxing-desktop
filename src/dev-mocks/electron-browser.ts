...
const MOCK_THEMES = [
  ...
  { key: 'codemaxxing', name: 'Codemaxxing', description: 'Signature midnight workbench — electric, focused, unmistakably ours', colors: { primary: '#6D8BFF', secondary: '#A855F7', muted: '#8C97B2', text: '#EEF2FF', userInput: '#A7F3D0', response: '#EEF2FF', tool: '#67E8F9', toolResult: '#9AA7C2', error: '#FB7185', success: '#34D399', warning: '#FBBF24', border: '#273552', suggestion: '#C084FC', bg: '#070A12', bgSubtle: '#0B101B' } },
...
export function installBrowserMock(): void {
  ...
  ;(window as any).electron = {
    ...
    browser: {
      onCommand: () => () => {},
      onOpen: (cb: (data: { sessionId: string | null }) => void) => {
        const w = window as unknown as { __fireBrowserOpen?: (sessionId?: string) => void }
        w.__fireBrowserOpen = (sessionId?: string) => cb({ sessionId: sessionId ?? null })
        return () => {}
      },
      sendResult: () => {},
      ready: () => {},
