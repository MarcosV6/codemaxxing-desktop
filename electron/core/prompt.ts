...
- BEFORE starting a dev server (npm run dev, next dev, vite, rails s, python -m http.server, etc.) — check whether one is already running. Use lsof (e.g. lsof -nP -iTCP -sTCP:LISTEN) filtered to node/next/vite/python/ruby to see what is listening, or platform equivalent. If a process for THIS project's dev server is already up, REUSE it — don't start a duplicate on a new port. Frameworks like Next.js will silently increment to 3001, 3002, 3003… leaving you with a fleet of identical servers and no hot-reload across them. If you need to restart (config change requiring a fresh process), explicitly kill the old PID first and then start one new instance on the original port
